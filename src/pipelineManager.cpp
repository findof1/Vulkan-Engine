...
void PipelineManager::createOffScreenRenderPass(VkDevice device, VkPhysicalDevice physicalDevice)
{
...
void PipelineManager::createColorIDRenderPass(VkDevice device, VkPhysicalDevice physicalDevice)
{
  VkAttachmentDescription colorAttachment{};
  ...
  if (vkCreateRenderPass(device, &renderPassInfo, nullptr, &colorIDRenderPass) != VK_SUCCESS)
  {
    throw std::runtime_error("failed to create colorID render pass!");
  }
}
...
void PipelineManager::createComputePipeline(VkDevice device)
{
  ...
  pipelineLayoutInfo.pSetLayouts = &descriptorManager.computeDescriptorSetLayout;
...
void PipelineManager::createGraphicsPipeline(VkDevice device)
{
  ...
  VkVertexInputBindingDescription particleBinding = Particle::getBindingDescription();
  auto particleAttributes = Particle::getAttributeDescriptions();

  VkPipelineVertexInputStateCreateInfo particleInputInfo{};
  particleInputInfo.sType = VK_STRUCTURE_TYPE_PIPELINE_VERTEX_INPUT_STATE_CREATE_INFO;
  particleInputInfo.vertexBindingDescriptionCount = 1;
  particleInputInfo.pVertexBindingDescriptions = &particleBinding;
  particleInputInfo.vertexAttributeDescriptionCount = static_cast<uint32_t>(particleAttributes.size());
  particleInputInfo.pVertexAttributeDescriptions = particleAttributes.data();

  VkPipelineInputAssemblyStateCreateInfo particleInputAssembly{};
  particleInputAssembly.sType = VK_STRUCTURE_TYPE_PIPELINE_INPUT_ASSEMBLY_STATE_CREATE_INFO;
  particleInputAssembly.topology = VK_PRIMITIVE_TOPOLOGY_POINT_LIST;
  particleInputAssembly.primitiveRestartEnable = VK_FALSE;
  ...
  pipelineInfo.pVertexInputState = &particleInputInfo;
  pipelineInfo.pInputAssemblyState = &particleInputAssembly;

  if (vkCreateGraphicsPipelines(device, VK_NULL_HANDLE, 1, &pipelineInfo, nullptr, &graphicsParticlePipeline) != VK_SUCCESS)
  {
    throw std::runtime_error("failed to create graphics pipeline!");
  }
...
{
  auto vertShaderCode = readFile("shaders/vertColorID.spv");
  auto fragShaderCode = readFile("shaders/fragColorID.spv");
  ...
  pushConstantRange.stageFlags = VK_SHADER_STAGE_VERTEX_BIT | VK_SHADER_STAGE_FRAGMENT_BIT;
  pushConstantRange.offset = 0;
  pushConstantRange.size = sizeof(ColorIDPushConstant);
  ...
  if (vkCreatePipelineLayout(device, &pipelineLayoutInfo, nullptr, &colorIDPipelineLayout) != VK_SUCCESS)
  {
    throw std::runtime_error("failed to create pipeline layout!");
  }
  ...
  pipelineInfo.layout = colorIDPipelineLayout;

  pipelineInfo.renderPass = colorIDRenderPass;
  pipelineInfo.subpass = 0;

  pipelineInfo.basePipelineHandle = VK_NULL_HANDLE;

  if (vkCreateGraphicsPipelines(device, VK_NULL_HANDLE, 1, &pipelineInfo, nullptr, &colorIDPipeline) != VK_SUCCESS)
  {
    throw std::runtime_error("failed to create graphics pipeline!");
  }
...
void PipelineManager::cleanup(VkDevice device)
{
  ...
  vkDestroyPipeline(device, graphicsParticlePipeline, nullptr);
